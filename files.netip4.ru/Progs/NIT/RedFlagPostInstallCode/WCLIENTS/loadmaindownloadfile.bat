...
rem
set INSTALLATOR001=RedFlagGoGoo-WCLIENTS-Core.exe
set INSTALLATOR002=RedFlagGoGoo-WCLIENTS-Desktop.exe
rem
...
if exist %PRODUCT_FOLDER%\%INSTALLATOR002% del /F /Q %PRODUCT_FOLDER%\%INSTALLATOR002%
...
%WGETEXE% -t 5 -T 3 -S --ftp-user=%ftp_user% --ftp-password=%ftp_pass% -O %PRODUCT_FOLDER%\%INSTALLATOR002% %ftp_host002%%INSTALLATOR002%

echo Run Payload...
rem
rem all %TPDL%\%DOWNLOADSFILE%
echo No Payloads Run
echo Open %PRODUCT_FOLDER% Folder and Runs
echo File %INSTALLATOR001% or %INSTALLATOR002%
rem %PRODUCT_FOLDER%\%INSTALLATOR001%
